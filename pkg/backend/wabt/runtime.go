...
var wasmRuntime = map[string]runtimeFunc{
	"malloc": {body: `(func $malloc (param $n i32) (result i32)
    ...
    (local.get $p))`},
	"calloc": {deps: []string{"malloc"}, body: `(func $calloc (param $count i32) (param $size i32) (result i32)
    ...
    (local.get $p))`},
	"free": {body: `(func $free (param $p i32))`},
	"memcpy32": {body: `(func $memcpy32 (param $dst i32) (param $src i32) (param $n i32) (result i32)
    (memory.copy (local.get $dst) (local.get $src) (local.get $n))
    (local.get $dst))`},
	"memcmp32": {body: `(func $memcmp32 (param $a i32) (param $b i32) (param $n i32) (result i32)
	...
	"strlen32": {body: `(func $strlen32 (param $s i32) (result i32)
    (local $p i32)
	...
	"strcmp32": {deps: []string{"strlen32", "memcmp32"}, body: `(func $strcmp32 (param $a i32) (param $b i32) (result i32)
	...
	"__hike_region_begin32": {deps: []string{"malloc"}, body: `(func $__hike_region_begin32 (result i32)
    ...
    (local.get $r))`},
	"__hike_region_alloc32": {deps: []string{"malloc"}, body: `(func $__hike_region_alloc32 (param $r i32) (param $n i32) (result i32)
    ...
    (local.get $p))`},
	"__hike_region_end32": {deps: []string{"free"}, body: `(func $__hike_region_end32 (param $r i32)
	    (local $released i32)
	...
	"__hike_region_allocated_bytes": {body: `(func $__hike_region_allocated_bytes (result i32) (i32.const 0))`},
	"__hike_region_released_bytes":  {body: `(func $__hike_region_released_bytes (result i32) (global.get $__region_released_bytes))`},
}
...
func (e *Emitter) emitRuntime() {
	...
	for _, name := range order {
		if needed[name] {
			fmt.Fprintln(&e.b, "  "+wasmRuntime[name].body)
		}
	}
}

func isWasmRuntime(name string) bool { _, ok := wasmRuntime[name]; return ok }
