...
func (g *CodeGenerator) emitPrologue() {
	...
	for _, fn := range g.semaCtx.Functions {
		if fn.IsExtern {
			if fn.Name == "malloc" || fn.Name == "free" || fn.Name == "calloc" || fn.Name == "strcmp" || fn.Name == "strlen" || fn.Name == "memcpy" || fn.Name == "memcmp" {
				continue
			}
			retType := "void"
...
func (g *CodeGenerator) emitAssignStmt(b *strings.Builder, s *ast.AssignStmt) {
	if len(s.Left) == 1 && len(s.Right) == 1 {
		...
		// 1. スライス/配列のインデックス代入: s[i] += val, s[i]++
		...
		// 3. 構造体フィールド代入: obj.field += val, obj.field++
...
func (g *CodeGenerator) resolveValue(b *strings.Builder, expr ast.Expression) (string, sema.Type) {
	...
	case *ast.MemberExpr:
		...
		// 構造体のポインタレジスタを特定 (値型変数の場合はシンボルのポインタを使用)
		objPtrReg := objReg
	...
	case *ast.SliceExpr:
		...
		// 文字列（string / i8*）のスライス: @hike_substr を呼び出し
		lowReg := "0"
		if e.Low != nil {
			lowReg, _ = g.resolveValue(b, e.Low)
		}
