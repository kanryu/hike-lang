...
| **3** | `int` | **`consumed`** | **The total number of source bytes successfully consumed from `src`.** |
...
| **3** | `int` | **`produced`** | **The total number of encoded data bytes written to `dst` (excluding the terminating null byte).** |
...
func main() int {
    ...
    // 3. Encode internal UTF-8 string into Shift_JIS cstring
    sample := "日本語文字列変換の実証。"
